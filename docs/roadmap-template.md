...
5. Customize milestones and dates to your org's planning cycle.
...
| Identify Federal-specific restrictions and document separately | Compliance team | Federal addendum to policy | [ ] |
...
| Action | Owner | Deliverable | Done |
|--------|-------|-------------|------|
| Configure org-managed API endpoints (AWS Bedrock or org proxy) | Platform team | API gateway/proxy running in dev | [ ] |
| Set up centralized API key management (AWS Secrets Manager) | Platform team | Keys provisioned for pilot teams | [ ] |
| Build devcontainer image with approved agent tooling | Platform team | Image published to Artifactory | [ ] |
| Configure secrets scanning on pilot repos (gitleaks or GitHub Advanced Security) | Security team | Scanning active on pilot repos | [ ] |
| Set up CODEOWNERS on `.github/workflows/` in pilot repos | Platform team | CODEOWNERS files committed | [ ] |
| Create reusable GitHub Actions for agent gates (lint, test, secrets scan) | Platform team | Actions published to shared repo or Artifactory | [ ] |

### Week 3-5: Context and Standards

| Action | Owner | Deliverable | Done |
|--------|-------|-------------|------|
| Write CLAUDE.md templates for pilot project types (service, library, IaC module) | Tech leads + platform team | Templates in shared repo | [ ] |
| Add CLAUDE.md to each pilot repo, customized for that project | Pilot team developers | CLAUDE.md committed in each pilot repo | [ ] |
| Create PR template with agent disclosure field | Platform team | PR template deployed to pilot repos | [ ] |
...
| Document Terraform conventions and IaC guardrails if IaC is in pilot scope | Infrastructure team | Terraform style guide | [ ] |
...
- [ ] Devcontainer images with agent tooling available in Artifactory
- [ ] CLAUDE.md and PR templates deployed to pilot repos
...
|---------|-----------|-----------|
| Q1 | Pilot tooling deployed (devcontainers, API gateway, key management). Agent gates in CI for pilot repos. | Agent adoption rate: 30% of pilot team developers active weekly |
| Q2 | Tooling available to all teams. Devcontainer images auto-updated via Artifactory pipeline. Agent auto-labeling deployed org-wide. | Agent adoption rate: 25% of all developers active weekly |
...
**Objective**: Establish, enforce, and refine policies for safe and compliant agent usage across commercial and Federal environments.

| Quarter | Milestone | KPI Target |
|---------|-----------|-----------|
| Q1 | Acceptable use policy published. Data classification matrix defined. Federal addendum written. API traffic routed through org proxy. | Policy compliance rate: >95% for pilot teams |
| Q2 | Policy enforced via technical controls (DLP, network boundaries, CODEOWNERS). Quarterly audit process established. Secrets scanning org-wide. | Policy compliance rate: >97% org-wide |
| Q3 | Exception process operational. Automated compliance scanning for agent-generated code. Federal-specific policy-as-code enforced. | Security finding rate in agent code: baseline established |
...
| Q3 | Advanced training: prompt engineering, CLAUDE.md authoring, agent-in-CI usage. Lunch-and-learn series on agent best practices. | 70% of developers completed at least basic training |
...
| Q2 | Expand to 5-8 teams based on pilot findings. Include at least one Federal team. Adjust tooling/policy based on pilot lessons. | Expanded teams onboarded within 2 weeks |
...
| Quarter | Milestone | KPI Target |
|---------|-----------|-----------|
| Q1 | API gateway with logging deployed. Devcontainer images in Artifactory. CI agent gates for pilot repos. Centralized key management. | Infrastructure operational for pilot |
...
| 2 | Code acceptance rate | Accepted suggestions / total suggestions | IDE telemetry (Copilot dashboard, Claude telemetry) | Monthly |
| 3 | PR merge time (agent vs. human) | Median PR open-to-merge, segmented by label | GitHub API, PR metrics dashboard | Monthly |
...
| 5 | Agent code rework rate | Agent PRs with follow-up fix within 14 days / total agent PRs | GitHub API, PR labels | Monthly |
| 6 | CI pipeline time impact | Agent step duration / total pipeline duration | GitHub Actions timing data | Monthly |
...
| 2 | Tooling deployed for pilot. Devcontainers in Artifactory. API gateway live. |
...
| 6 | Expanded to 5-8 teams. Federal team included. Org-wide secrets scanning. |
...
| Federal compliance review delays tooling rollout | High | Medium | Engage compliance in Week 1. Run Federal approval in parallel with commercial deployment. |
